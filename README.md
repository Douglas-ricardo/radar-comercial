...
- [Módulos do sistema](#módulos-do-sistema)
  ...
  - [Disparo automático ao cliente final](#disparo-automático-ao-cliente-final)
...
- **Notificações automáticas ao vendedor** — digest diário por email e WhatsApp com as melhores oportunidades do dia
- **Disparo automático ao cliente final** — reativa clientes inativos enviando WhatsApp (do número do vendedor, via Evolution API) e email com mensagem personalizada por IA, com opt-out e revisão antes do envio
- **Ingestão automática** — integração com ERPs (Omie, Bling, Conta Azul) via n8n usando API Key própria por empresa, ou pull agendado de Google Sheets
...
| **Billing** | Stripe (Checkout + Webhooks, per-seat) |
| **Email** | Resend |
| **WhatsApp (vendedor)** | WhatsApp Cloud API (Meta) — digest diário |
| **WhatsApp (cliente final)** | Evolution API (self-hosted, número do vendedor) |
| **IA** | Claude Haiku (Anthropic) — mensagens personalizadas |
| **Object storage** | Cloudflare R2 / S3-compatível (degrada p/ disco local) |
| **PDF** | fpdf2 (relatório de insights) |
...
### Notificações diárias ao vendedor
```
Celery Beat 08:00 BRT → send_daily_notifications
  ...
  → Envia texto via WhatsApp Cloud API (Meta) (máx 5 oportunidades)
```

### Disparo automático ao cliente final
```
Celery Beat horário → send_daily_outreach (filtra empresas pelo send_hour)
  → Para cada empresa com auto-envio ativo:
    → Seleciona clientes inativos (at_risk/lost) com contato, sem opt-out, não enviados em 24h
    → IA (Claude Haiku) gera mensagem de reativação por cliente
    → WhatsApp via Evolution API (número do vendedor) + Email via Resend
    → Intervalo aleatório entre envios (anti-ban) + marca oportunidade como "contacted"
```
...
- **Plano inicial** — escolha entre Gratuito, Profissional (R$199/mês) ou Enterprise (R$499/mês)
...
| `phone` *(opcional)* | `telefone`, `celular`, `whatsapp`, `fone`, `contato` |
| `email` *(opcional)* | `email`, `e-mail`, `mail` |

> As colunas `telefone`/`email` são **opcionais** e habilitam o disparo automático ao cliente final. O telefone é normalizado para E.164 brasileiro (`+55…`).
...
Um cliente é considerado **inativo (churned)** quando sua última compra ocorreu há mais de **60 dias** em relação à **data de referência**.

A data de referência é **hoje** quando o arquivo está atualizado (≤7 dias de defasagem); se o CSV estiver defasado (>7 dias), a referência passa a ser a data mais recente do próprio arquivo, e o `summary.dataFreshness` sinaliza isso para o frontend exibir um banner ("Dados até DD/MM"). Isso evita falsos churns com planilhas antigas.
...
#### Exportar PDF
Gera um relatório completo em PDF no backend (`fpdf2`, via `GET /api/insights/{company_id}/report`) com KPIs, oportunidades, top clientes, gaps de produto e evolução mensal — respeitando o período selecionado. O frontend baixa o arquivo direto.
...
> O WhatsApp ao vendedor usa a **WhatsApp Cloud API (Meta)** — requer `WHATSAPP_API_TOKEN` e `WHATSAPP_PHONE_NUMBER_ID`.

---

### Disparo automático ao cliente final

**Página:** `/dashboard/disparo`

Reativa clientes inativos enviando mensagem direto para eles (não para o vendedor). O WhatsApp sai do **número do próprio vendedor** via Evolution API — o cliente reconhece quem está falando.

#### Conexão do WhatsApp
- Botão **"Conectar via QR Code"** cria uma instância no Evolution e exibe o QR
- O vendedor escaneia em **WhatsApp → Aparelhos conectados** (igual ao WhatsApp Web)
- A sessão persiste (reconecta sozinha após restart); o painel faz polling de status

#### Canais e regras
| Configuração | Descrição |
|---|---|
| **WhatsApp / Email** | Toggles de canais ativos |
| **Nome na assinatura** | Nome do vendedor nas mensagens e emails |
| **Horário de envio (BRT)** | Hora do disparo automático diário |
| **Limite de mensagens/dia** | Teto anti-ban |
| **Valor mínimo da oportunidade** | Filtra quem recebe |
| **Disparo automático diário** | Liga o envio agendado |

#### Contatos e opt-out
Tabela com todos os clientes: completa telefone/email faltantes manualmente e marca quem **não** deve receber (opt-out). Contatos manuais e opt-out são preservados entre re-uploads.

#### Revisar e enviar
O botão **"Revisar e enviar"** mostra a **mensagem real** (gerada por IA para um cliente elegível) num modal antes de confirmar — o vendedor vê exatamente o que será enviado. Ao confirmar, dispara em segundo plano com intervalo aleatório entre envios.

#### Segurança e LGPD
- Só envia para clientes **at_risk/lost** com contato e sem opt-out
- **Dedup 24h**: não reenvia para o mesmo cliente no mesmo dia
- Cada envio é registrado em `OutreachLog` (sem o texto, por LGPD)
- Envio bem-sucedido marca a oportunidade como `contacted` na Carteira
...
|---|---|---|---|
| **Preço** | R$ 0/mês | R$ 199/mês | R$ 499/mês |
...
| `telefone` | Texto | Não | `(11) 98238-7185`, `11987654321` — habilita disparo WhatsApp |
| `email` | Texto | Não | `cliente@email.com` — habilita disparo por email |

**Exemplo:**
```csv
data,cliente,produto,quantidade,valor,telefone,email
01/01/2024,Empresa ABC Ltda,Produto Premium,5,2500.00,(11) 98238-7185,abc@email.com
15/01/2024,Comércio XYZ,Serviço Mensal,1,890.00,,
20/02/2024,Indústria Beta,Produto Standard,12,360.00,11999998888,beta@email.com
```
...
| `phone` | String nullable | Telefone E.164 do cliente (PII — disparo WhatsApp). Preservado entre uploads |
| `email` | String nullable | Email do cliente (PII — disparo email). Preservado entre uploads |
| `contact_opt_out` | Boolean | Excluído do disparo automático (decisão do vendedor) |
...
> Quando o disparo automático envia com sucesso a um cliente, a oportunidade correspondente é marcada como `contacted` automaticamente.

### OutreachConfig (outreach_configs)

Configuração de disparo ao cliente final, um registro por empresa.

| Campo | Tipo | Descrição |
|---|---|---|
| `company_id` | UUID FK unique | Empresa |
| `auto_send_enabled` | Boolean | Liga o envio automático diário |
| `whatsapp_enabled` / `email_enabled` | Boolean | Canais ativos |
| `evolution_instance` | String | Nome da instância Evolution (= company_id) |
| `whatsapp_status` | String | `disconnected` / `connecting` / `connected` |
| `whatsapp_number` | String nullable | Número do vendedor conectado |
| `sender_name` | String nullable | Assinatura nas mensagens/emails |
| `send_hour` | Integer | Hora BRT do envio automático |
| `daily_limit` | Integer | Teto de mensagens por dia (anti-ban) |
| `min_opportunity_value` | Float | Valor mínimo para disparar |

### OutreachLog (outreach_logs)

Registro de cada envio ao cliente final — base do dedup 24h, métricas e auditoria.

| Campo | Tipo | Descrição |
|---|---|---|
| `company_id` | UUID FK | Empresa |
| `customer_hash` | String | Cliente destino |
| `channel` | String | `whatsapp` / `email` |
| `status` | String | `sent` / `failed` / `skipped` |
| `sent_at` | DateTime | Momento do envio (índice p/ dedup) |

> Não armazena o texto da mensagem (LGPD) — apenas metadados.
...
| GET | `/api/insights/{company_id}/report` | Relatório em PDF do período (`?date_range=6m`) |

### Opportunities (IA)
| Método | Rota | Descrição |
|---|---|---|
| POST | `/api/opportunities/{id}/generate-message` | Gera mensagem personalizada via Claude Haiku |
...
| Método | Rota | Descrição |
|---|---|---|
| GET / PATCH | `/api/outreach/config` | Buscar / atualizar config de disparo |
| POST | `/api/outreach/whatsapp/connect` | Cria instância Evolution e retorna QR Code |
| GET | `/api/outreach/whatsapp/status` | Estado da conexão WhatsApp |
| POST | `/api/outreach/whatsapp/disconnect` | Desconecta o WhatsApp |
| GET | `/api/outreach/contacts` | Lista clientes com contato, opt-out e último envio |
| PATCH | `/api/outreach/contacts/{customer_hash}` | Edita telefone/email/opt-out de um cliente |
| GET | `/api/outreach/preview` | Prévia da mensagem (sem enviar) |
| POST | `/api/outreach/send-now` | Dispara o lote do dia imediatamente |
...
# ── Email (Resend) ────────────────────────────────────────────────────────────
RESEND_API_KEY=re_...
RESEND_FROM_EMAIL=Radar Comercial <noreply@seudominio.com>   # domínio precisa verificado

# ── WhatsApp ao VENDEDOR — Cloud API (Meta) ───────────────────────────────────
WHATSAPP_API_TOKEN=EAA...         # token do app Meta
WHATSAPP_PHONE_NUMBER_ID=123...   # ID do número (não o número)

# ── WhatsApp ao CLIENTE FINAL — Evolution API (self-hosted) ───────────────────
EVOLUTION_API_URL=http://localhost:8080
EVOLUTION_API_KEY=                # você define: openssl rand -hex 24

# ── IA (Claude Haiku) ─────────────────────────────────────────────────────────
ANTHROPIC_API_KEY=sk-ant-...      # mensagens personalizadas (fallback estático se ausente)

# ── Object storage (opcional — degrada p/ disco local) ────────────────────────
R2_ENDPOINT_URL=
R2_ACCESS_KEY_ID=
...
> **Twilio foi removido.** WhatsApp ao vendedor = Cloud API (Meta); ao cliente final = Evolution.
...
# Em outro terminal: worker Celery (macOS exige --pool=threads — ver nota abaixo)
celery -A app.core.celery_app.celery_app worker --loglevel=info --pool=threads --concurrency=4

# Em outro terminal: scheduler de notificações + disparo
celery -A app.core.celery_app.celery_app beat --loglevel=info
```

> **macOS:** use `--pool=threads` no worker. O pool padrão (prefork) faz `fork()` que herda
> o threadpool nativo do polars/pyarrow → SIGSEGV ao processar arquivo. Em Linux/produção
> o prefork funciona normalmente.

### Evolution API (WhatsApp ao cliente final)

```bash
cd backend
# Sobe Evolution + postgres/redis isolados (não conflita com seus serviços locais)
EVOLUTION_API_KEY=<sua_key> docker compose -f docker-compose.evolution.yml up -d
# API: http://localhost:8080  ·  Manager visual: http://localhost:8080/manager
```
...
### ✅ Concluído
- **IA para mensagem personalizada** — `POST /api/opportunities/{id}/generate-message` (Claude Haiku)
- **Disparo automático ao cliente final** — WhatsApp (Evolution, número do vendedor) + Email, com IA, opt-out, dedup 24h e sync com a Carteira
- **Cobrança per-seat** — `quantity` = nº de usuários no Stripe
- **Object storage R2/Spaces** — desacopla API↔worker (degrada p/ disco local)
- **Google Sheets** — pull agendado de planilha de vendas
- **Relatório PDF** — gerado no backend (fpdf2)
- **Hardening** — revogação JWT por `credential_version`, força de senha, container não-root, lock de ETL

### ⏳ Em aberto
- **Conectores de ERP via n8n** — templates de workflow para Omie, Conta Azul, Bling
- **`ml/`** — modelos preditivos locais (hoje stubs vazios; IA usa Claude API)
- **SSO** para plano Enterprise
- **Verificação Meta / domínio Resend** — passos operacionais para produção (não-código)
